...
function onApply() {
    ...
    const game = new TagGame(num_vertices);
    engine = new TagEngine("board", "status");
    engine.setGame(game);
    engine.setPlayers([first,second]);
    engine.play();
    document.getElementById("gametitle").innerHTML = `<h3>TAG on <i>K</sub>${num_vertices}</sub></i><br>${first == 'human' ? 'human' : 'bot'} vs. ${second == 'human' ? 'human' : 'bot'}</h3>`;
}
